...
def _make_session(year, understat_league='EPL'):
    """
    ...
    league_url = f'https://understat.com/league/{understat_league}/{year}'
    try:
...
def fetch_season_xg(year, session=None, league: LeagueConfig | str = 'epl', team_map=None):
    """
    ...
    understat_league = config.sources.understat_league or config.display_name
    team_map = team_map or config.team_aliases or UNDERSTAT_TEAM_MAP
    if session is None:
        session = _make_session(year, understat_league)

    api_url = f'https://understat.com/getLeagueData/{understat_league}/{year}'
    try:
        resp = session.get(
            api_url,
            headers={
                'X-Requested-With': 'XMLHttpRequest',
                'Referer': f'https://understat.com/league/{understat_league}/{year}',
            },
            timeout=20,
        )
    ...
    for item in dates:
        ...
        try:
            ...
            matches.append({
                ...
                'HomeTeam':             team_map.get(home_title, home_title),
                'AwayTeam':             team_map.get(away_title, away_title),
...
def main(league: LeagueConfig | str = 'epl'):
    ...
    for year in seasons:
        ...
        session = _make_session(year, config.sources.understat_league or config.display_name)
        matches = fetch_season_xg(year, session=session, league=config)
