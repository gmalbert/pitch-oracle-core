...
BZZOIRO_TEAM_MAP = {
    ...
    "CD Aves": "AVS",
    "AVS Futebol SAD": "AVS",
    "Associação Desportiva de Alverca": "Alverca",
    "FC Arouca": "Arouca",
}
...
def get_league_id(league: LeagueConfig | str = 'epl') -> int:
    """Discover and cache a Bzzoiro league ID from a shared league config."""
    config = get_league_config(league) if isinstance(league, str) else league
    if config.key in _LEAGUE_IDS:
        return _LEAGUE_IDS[config.key]
    ...
    for league in cached:
        if league.get("name") == config.display_name:
            _LEAGUE_IDS[config.key] = league["id"]
            return _LEAGUE_IDS[config.key]

    raise RuntimeError(f"{config.display_name} not found in Bzzoiro leagues list.")


def get_pl_league_id() -> int:
    """Backward-compatible EPL alias."""
    return get_league_id('epl')


def get_events(league: LeagueConfig | str = 'epl', date_from=None, date_to=None, status=None) -> list[dict]:
    """League-neutral events API; legacy EPL helper remains an alias below."""
    config = get_league_config(league) if isinstance(league, str) else league
    ...
    cache_key = f"events_{config.key}_{d_from}_{d_to}_{status or 'all'}.json"
    cached = _load_cached(_cache_path(cache_key), _TTL["events"])
    if cached is not None:
        return cached
    params = {"league": league_id, "date_from": d_from, "date_to": d_to, "tz": "Europe/London"}
    if status:
...
def get_pl_events(
    ...
    """
    return get_events('epl', date_from, date_to, status)
