...
LEAGUE_ID = int(os.getenv("PITCH_ORACLE_API_FOOTBALL_LEAGUE_ID", "39"))
...
CACHE_DIR = os.path.join(os.getenv("PITCH_ORACLE_DATA_DIR", "data_files"), "api_cache")
DATA_DIR = os.getenv("PITCH_ORACLE_DATA_DIR", "data_files")

os.makedirs(CACHE_DIR, exist_ok=True)


def configure(league: LeagueConfig | str = "epl") -> None:
    """Set API-Football's league scope for a consumer before fetching."""
    global LEAGUE_ID
    config = get_league_config(league) if isinstance(league, str) else league
    if config.sources.api_football_league_id is None:
        raise ValueError(f"No API-Football league ID configured for {config.key}")
    LEAGUE_ID = config.sources.api_football_league_id
